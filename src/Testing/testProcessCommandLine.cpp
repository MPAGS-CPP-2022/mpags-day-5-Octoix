...
TEST_CASE("Multiple ciphers command line argument updated correctly")
{
    ProgramSettings settings{false, false, "", "", {}, {}, CipherMode::Encrypt, 1u};
    ...
    REQUIRE(res);
    REQUIRE(settings.nExpectedCiphers == 3u);
}

TEST_CASE("Negative values of multi-cipher are processed as positive")
{
    ProgramSettings settings{false, false, "", "", {}, {}, CipherMode::Encrypt, 1u};
    const std::vector<std::string> cmdLine{"mpags-cipher", "--multi-cipher", "-3"};
    const bool res{processCommandLine(cmdLine, settings)};
